...
for beta in S:
    ...
    with tf.Session() as sess:
        ...
        print("Final test accuracy decay parameter:", beta, ':', test_accs[-1])


        # plot learning curves
    plt.figure('Train Error vs Epochs')
    plt.plot(range(EPOCHS), train_errors, label= 'Decay of ' + str(beta))
    plt.xlabel(str(EPOCHS) + ' iterations')
    plt.ylabel('Train Error')
    plt.legend()
    plt.figure('Test Accuracy vs Epochs')
    plt.plot(range(EPOCHS), test_accs, label= 'Decay of ' + str(beta))
