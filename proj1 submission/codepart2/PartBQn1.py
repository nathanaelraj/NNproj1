...
with tf.Session() as sess:
	...
	predictions = sess.run(y,{ x: testX[:50]})
# plot learning curves
plt.figure(1)
plt.plot(range(epochs), train_err)
plt.xlabel(str(epochs) + ' iterations')
plt.ylabel('Train Error in RMSE, units = thousands')

list1, list2 = (list(t) for t in zip(*sorted(zip( testY[:50],predictions))))
...
plt.figure(2)
plt.plot(range(50), list1,'ro')
plt.plot(range(50), list2,'bo')
