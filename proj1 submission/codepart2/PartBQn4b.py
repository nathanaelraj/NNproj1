...
for dropout in range(2):
	for layers in range(1,4):
		...
		if(layers == 1):
			...
			if(dropout == 1):
				drop_out = tf.nn.dropout(output_1, keep_prob)
			#final output layer
		...
		if(layers ==2):
			...
			if(dropout == 1):
				drop_out = tf.nn.dropout(output_2, keep_prob)
		...
		if(layers ==3):
			...
			if(dropout == 1):
				drop_out = tf.nn.dropout(output_3, keep_prob)
		...
		plt.plot([i for i in range(len(test_errors))], test_errors,c=c,label=  str(layers)+ ' hidden layer' +' drop out at ' + str(dropout) )
plt.legend()
...
plt.plot([3,4,5],finalResult[:3],'ro')
plt.plot([3,4,5],finalResult[3:],'bo')
plt.xlabel('num of layers (Red = no dropout, Blue = dropout)')
