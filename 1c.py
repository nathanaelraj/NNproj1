...
for NUM_HIDDEN in S:
    ...
    with tf.Session() as sess:
        ...
        print("Final test accuracy for number of hidden neurons:", NUM_HIDDEN, ':', test_accs[-1])
        duration = time.time() - start_time
        durations.append(duration/EPOCHS)

        # plot learning curves
    plt.figure('Train Error vs Epochs')
    plt.plot(range(EPOCHS), train_errors, label= str(NUM_HIDDEN) + " hidden neurons")
    plt.xlabel(str(EPOCHS) + ' epochs')
    plt.ylabel('Train error')
    plt.legend()
    plt.figure('Test Accuracy vs Epochs')
    plt.plot(range(EPOCHS), test_accs, label= str(NUM_HIDDEN) + " hidden neurons")
    plt.xlabel(str(EPOCHS) + ' epochs')
    plt.ylabel('Test accuracy')
    plt.legend()

plt.figure('Average Time Taken for one epoch vs No. of Hidden Neurons')
plt.plot(S, durations)
